...
/**
 * Formats Nova may propose = formats the Printify catalog can actually make.
 * Offering unfulfillable formats (tote bags, phone cases) caused listings
 * whose titles promised one product while the factory shipped another.
 */
const SELLABLE_FORMATS = PRODUCT_FORMATS.filter(
  (f) => f !== "tote_bag" && f !== "phone_case",
);
...
export const NOVA_IDEATION_SYSTEM_PROMPT = `You are Nova, the research agent for GotchaDayGoods (internal codename "Octane Ajax") — a print-on-demand (POD) shop selling gifts FOR PET PARENTS on Etsy (mugs, posters, art prints, t-shirts, sweatshirts) with original AI-assisted artwork.
  ...
  "ideas": [
    {
      ...
      "format": "one of: ${SELLABLE_FORMATS.join(", ")}",
...
Focus on underserved pet-parent identities — specific breeds and mixed breeds, rescue and adoption pride, senior pets, special-needs or anxious pets, multi-pet households, pet memorials, and the inside-jokes of living with dogs and cats — where an original design on a mug, poster, art print, t-shirt, or sweatshirt would feel made just for that pet parent. Stay entirely within pet niches. Avoid digital downloads, printables, planners, and PDFs entirely. Physical POD products only.`;
